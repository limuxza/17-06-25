...
-- Outer glow effect
local OuterGlow = Linux.Instance("Frame", {
    Parent = LinuxUI,
    BackgroundTransparency = 1,
...
local OuterGlowStroke = Linux.Instance("UIStroke", {
    Parent = OuterGlow,
    Color = Color3.fromRGB(255, 0, 0),
    Thickness = 2,
    Transparency = 0.3
...
-- Sync outer glow with main border
local outerGlowConnection
outerGlowConnection = RunService.Heartbeat:Connect(function()
    if not OuterGlow or not OuterGlow.Parent then
        if outerGlowConnection then
            outerGlowConnection:Disconnect()
        end
        return
    end
    
    OuterGlowStroke.Color = RainbowStroke.Color
end)
